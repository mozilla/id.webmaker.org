...
class IdentityDatabase {
  ...
  forbidden(reason, details) {
    return Boom.forbidden(reason, details);
  }
  ...
  getAuthCode(code) {
    return this.dbConnection('auth_codes')
      .select('auth_code', 'client_id', 'scopes', 'created_at', 'expires_at')
  ...
  deleteAuthCode(auth_code) {
    return this.dbConnection('auth_codes')
      ...
      .del()
      .returning(['auth_code', 'client_id', 'scopes', 'created_at', 'expires_at']);
  }
  ...
  insertUser(email, username, pref_locale, salted_hash) {
    return this.dbConnection('users')
      ...
      .returning(['id', 'username', 'pref_locale', 'pref_locale']);
  }
  ...
  getResetCode(code, user_id) {
    return this.dbConnection('reset_codes')
      .select()
      .where('code', '=', code);
  }

  invalidateUserResetCodes(uid) {
    return this.dbConnection('reset_codes')
      .where('user_id', '=', uid)
      .andWhere('valid', '=', true)
      .update({
  ...
  setPassword(user_id, salted_hash) {
    return this.dbConnection('users')
